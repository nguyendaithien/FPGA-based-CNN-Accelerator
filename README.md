...
Firstly, let unzip parameter file in ./data/wgt.zip.
```sh
cd ./data
unzip wgt.zip
```
...
make all
```
Let keep patient because simulation can make long time to complete (from 10 to 15 minutes). During simulation time you can see that we print result of OFM on screen like below: 
```sh
...
```
After complely finish, a golden output can be loaded to compare with RTL result, if both values match, the simulation will report like below: 
```sh
██████╗  █████╗ ███████╗███████╗    ████████╗███████╗███████╗████████╗                                                                               
██╔══██╗██╔══██╗██╔════╝██╔════╝    ╚══██╔══╝██╔════╝██╔════╝╚══██╔══╝                                                                               
██████╔╝███████║███████╗███████╗       ██║   █████╗  ███████    ██║
